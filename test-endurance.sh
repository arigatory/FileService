...
# РАБОЧИЙ ТЕСТ НА ВЫНОСЛИВОСТЬ: 5 циклов × 1GB файлы

set -e

echo "💪 ТЕСТ НА ВЫНОСЛИВОСТЬ: 5 циклов × 1GB файлы"
echo "=============================================="
echo "Лимит памяти: 500MB на контейнер"
echo "Общий объем: 5GB (по циклам)"
echo ""
...
TEST_DIR="/tmp/endurance_working"
TOTAL_CYCLES=5
success_count=0
failed_count=0
total_upload_time=0
...
for ((cycle=1; cycle<=TOTAL_CYCLES; cycle++)); do
    ...
    cycle_start=$(date +%s)
    filename="$TEST_DIR/endurance_${cycle}_1gb.txt"
    
    # 1. Создание файла
    echo -n "  📁 Создаем файл 1GB... "
    if dd if=/dev/zero of="$filename" bs=1M count=1024 2>/dev/null; then
        ...
        response=$(curl -s -X POST "$CLIENT_APP_URL/api/files/upload" \
            -F "file=@$filename" \
            -F "tags=endurance-test,cycle-$cycle,1gb")
        upload_end=$(date +%s)
