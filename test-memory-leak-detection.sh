...
echo "🔍 ИНТЕНСИВНЫЙ ТЕСТ ОБНАРУЖЕНИЯ УТЕЧЕК ПАМЯТИ (БЕЗ ПАУЗ)"
echo "======================================================="
echo "📁 Фаза 1: ${SMALL_FILES_COUNT} файлов по ${SMALL_FILE_SIZE}MB (${CONCURRENT_REQUESTS} одновременно)"
echo "📁 Фаза 2: Большие файлы 1GB, 2GB, 3GB, 5GB (без пауз между циклами)"
echo "💾 Лимит памяти: 400MB на контейнер fileservice"
echo "🎯 Цель: Протестировать реальную production нагрузку"
echo "⚡ Интенсивность: Максимальная - имитация реального трафика"
...
CONCURRENT_REQUESTS=10  # Увеличено до 10 одновременных запросов для реальной нагрузки
